...
/* libretro pixel formats are 0RGB1555=0, XRGB8888=1 and RGB565=2.
 * Using 1 here made gpSP submit 32-bit frames while video_refresh_cb read
 * them as 16-bit pixels, which results in a black/corrupt game screen. */
...
Java_com_emu_gba_GBAEngine_nativeInit(JNIEnv *env, jobject obj, jstring soPath) {
    ...
    if (!libhandle) {
        LOGE("Could not load core: %s", dlerror());
        return JNI_FALSE;
    }
    ...
    framebuffer = (uint32_t*)calloc(240 * 160, sizeof(*framebuffer));
    if (!framebuffer) {
        LOGE("Could not allocate framebuffer");
        p_retro_deinit();
        dlclose(libhandle);
        libhandle = NULL;
        return JNI_FALSE;
...
Java_com_emu_gba_GBAEngine_nativeGetFramebuffer(JNIEnv *env, jobject obj, jintArray buf) {
    if (!framebuffer || fb_width != 240 || fb_height != 160) return JNI_FALSE;
    int size = 240 * 160;
