...
static uint32_t rc_read_memory(uint32_t address, uint8_t* buffer,
    ...
    (void)client;
        ...
        /* VALUE_LOG2: catat semua address EWRAM (unik) yang dibaca rcheevos */
    if (address < 0x40000 && g_mem_region_count > 0) {
        static uint32_t logged_addr[300];
        static int log_count = 0;
        int already = 0;
        for (int j = 0; j < log_count; j++) {
            if (logged_addr[j] == address) { already = 1; break; }
        }
        if (!already && log_count < 300) {
            ...
            for (int i = 0; i < g_mem_region_count; i++) {
                if (g_mem_regions[i].start == 0x02000000 &&
                    ...
                    uint32_t v = 0;
                    memcpy(&v,
                           g_mem_regions[i].ptr + g_mem_regions[i].offset + address,
