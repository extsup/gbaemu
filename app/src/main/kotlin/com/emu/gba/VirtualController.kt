...
class VirtualController(context: Context) : View(context) {
    ...
    private val prefs: SharedPreferences = context.getSharedPreferences("GBAemuPrefs", Context.MODE_PRIVATE)
    var editMode = false

    companion object {
        private const val POSITION_VERSION = "v2"
    ...
    private fun initDefaultSizes(w: Int, h: Int) {
        if (w == 0 || h == 0) return
        // Size controls from width so they remain round and do not consume
        // the game area on a tall portrait display.
        val bw = w * 0.15f; val bh = bw
        val lbw = w * 0.18f; val lbh = w * 0.12f
        val sbw = w * 0.18f; val sbh = w * 0.12f
    ...
    private fun defaultCY(name: String, w: Int, h: Int): Float {
        val bh = w * 0.15f; val pad = h * 0.03f
        val portrait = h > w
        return when (name) {
            "L", "R" -> if (portrait) h * 0.73f else h * 0.58f
            "UP" -> h - bh * 2.5f - pad
            "DOWN" -> h - bh * 0.5f - pad
            "LEFT", "RIGHT", "A" -> h - bh * 1.5f - pad
            "B" -> h - bh * 0.5f - pad
    ...
    private fun loadPositions(w: Int, h: Int) {
        for (name in BUTTON_NAMES) {
            btnCX[name] = prefs.getFloat("btn_cx_${POSITION_VERSION}_$name", defaultCX(name, w, h))
            btnCY[name] = prefs.getFloat("btn_cy_${POSITION_VERSION}_$name", defaultCY(name, w, h))
        }
    }

    private fun savePosition(name: String) {
        prefs.edit().putFloat("btn_cx_${POSITION_VERSION}_$name", btnCX[name] ?: 0f)
            .putFloat("btn_cy_${POSITION_VERSION}_$name", btnCY[name] ?: 0f).apply()
    }
    ...
    fun resetPositions() {
        ...
        for (name in BUTTON_NAMES) {
            editor.remove("btn_cx_${POSITION_VERSION}_$name")
                .remove("btn_cy_${POSITION_VERSION}_$name")
        }
